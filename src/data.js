import React from "react";

export const reasonsData = [
  {
    title: "Reduce el riesgo de infección",
    description:
      "Las incisiones al ser de menor calibre, reducen la superficie de contacto con el medioe externo, evitando la exposición de las cavidades internas a microorganismos existentes en el medio ambiente. ",
    icon: (
      <svg
        id="Capa_1"
        enableBackground="new 0 0 484.559 484.559"
        height="100"
        viewBox="0 0 484.559 484.559"
        width="120"
        xmlns="http://www.w3.org/2000/svg">
        <g>
          <g id="XMLID_107_">
            <g>
              <path
                d="m484.426 222.033c1.77 22.9-14.34 43.23-35.97 45.4-16.56 1.66-31.54-7.81-38.46-22.6l-19.63 1.73-14.82 7.15c-3.39 1.63-5.67 4.87-6.19 8.59-1.4 10.03-3.94 19.69-7.47 28.87-1.2 3.11-.99 6.58.64 9.48l5.28 9.44 14.68 9.85c7.74-2.96 16.75-2.25 24.16 2.72 12.56 8.43 15.57 25.78 6.23 37.99-8.04 10.52-22.99 13.41-34.37 6.65-7.25-4.3-11.63-11.44-12.7-19.07l-16.27-10.91-10.24-1.25c-3.52-.43-7.02.78-9.58 3.23-8.64 8.23-18.4 15.3-29.01 20.96-3.57 1.9-5.86 5.55-6.11 9.58l-1.3 20.74 6.44 25.55c10.96 3.06 20.02 11.75 23 23.57 4.5 17.87-6.6 36-24.7 40.03-16.33 3.64-33.08-5.88-38.27-21.79-4.04-12.42-.33-25.26 8.21-33.65l-6.47-25.7-9.99-16.06c-2.68-4.31-7.42-6.86-12.5-6.85-.03 0-.05 0-.08 0-8.75 0-17.31-.87-25.59-2.5-4.93-.98-10.02.58-13.48 4.23l-13.2 13.93-14.78 30.66c3.46 7.03 3.77 15.52.11 23.13-6.58 13.64-23.36 19.06-36.77 11.52-11.48-6.47-16.45-20.75-11.45-32.95 3.41-8.32 10.48-13.88 18.48-15.74l14.87-30.86 2.92-20.71c.53-3.82-.93-7.67-3.93-10.09-11.83-9.6-21.97-21.19-29.9-34.27-1.97-3.25-5.51-5.22-9.31-5.25l-20.68-.2-22.64 7.12c-1.59 14.08-11.23 26.59-25.61 31.11-19.32 6.08-39.9-4.54-46.17-23.74-6.27-19.19 4.32-40.18 23.49-46.52 13.42-4.44 27.52-.75 37.06 8.32l24.03-7.56 17.55-12.51c2.61-1.85 4-4.95 3.74-8.13-.29-3.53-.44-7.09-.44-10.69 0-19.56 4.3-38.13 11.98-54.83 1.12-2.45 1.09-5.27-.09-7.69l-5.2-10.63-14.11-10.65c-7.9 2.53-16.85 1.32-23.97-4.05-12.08-9.12-14.12-26.61-4.12-38.28 8.61-10.05 23.7-12.11 34.69-4.73 7 4.7 10.98 12.06 11.62 19.74l15.64 11.8 15.76 2.81c11.47-10.65 24.84-19.28 39.53-25.33l3.7-22.48-4.67-23.22c-13.82-3.56-25.04-14.15-28.06-28.27-4.26-19.95 9.45-39.38 30.63-43.39 21.17-4.01 41.79 8.9 46.05 28.85 3.19 14.93-3.68 29.56-16.22 37.66l4.41 21.9 10.83 17.28c.71 1.13 1.95 1.81 3.28 1.81h.02c24.8 0 48.02 6.9 67.85 18.88 1.09.65 2.43.74 3.58.2l16.96-7.86 19.8-21.42c-1.19-7.68 1-15.82 6.69-21.97 10.28-11.11 27.88-11.39 38.5-.28 9.31 9.75 9.67 25.3.83 35.48-5.72 6.59-13.93 9.63-21.95 9.03l-19.8 21.42-6.43 17.3c-.45 1.2-.25 2.54.5 3.58 7.81 10.72 14.04 22.66 18.34 35.48 1.19 3.52 3.99 6.26 7.57 7.27l16.1 4.51 20.92-1.85c5.13-14.79 17.94-25.98 33.67-27.56 21.63-2.17 40.6 14.64 42.36 37.54z"
                fill="#eb5569"
              />
            </g>
          </g>
          <g id="XMLID_8_">
            <g>
              <circle cx="238.936" cy="243.983" fill="#f58a97" r="97.52" />
            </g>
          </g>
          <path
            d="m171.14 243.983c0-48.807 35.852-89.238 82.658-96.395-4.846-.741-9.809-1.125-14.862-1.125-53.86 0-97.52 43.66-97.52 97.52s43.66 97.52 97.52 97.52c5.053 0 10.016-.384 14.862-1.125-46.805-7.158-82.658-47.589-82.658-96.395z"
            fill="#f07281"
          />
          <g fill="#eb5569">
            <circle cx="74.853" cy="38.921" r="27.771" />
            <circle cx="67.262" cy="404.986" r="18.343" />
            <circle cx="404.051" cy="435.118" r="27.018" />
            <circle cx="209.002" cy="217.089" r="25.189" />
            <circle cx="229.222" cy="295.328" r="17.145" />
            <circle cx="283.058" cy="243.983" r="14.397" />
          </g>
        </g>
      </svg>
    ),
  },
  {
    title: "Menos cicatrices externas",
    description:
      "Debido a que la cirugía convencional ha sustituído las largas cicatrices propias de la cirugía tradicional por pequeños puntos de incisión.",
    icon: (
      <svg
        id="Capa_1"
        enableBackground="new 0 0 512 512"
        height="100"
        viewBox="0 0 512 512"
        width="120"
        xmlns="http://www.w3.org/2000/svg">
        <g>
          <path d="m431.921 179.18c0-25.215-21.153-45.729-47.155-45.729-6.103 0-11.94 1.13-17.299 3.186v-60.113c0-25.214-21.153-45.728-47.155-45.728-6.648 0-12.98 1.342-18.723 3.759-5.154-19.832-23.702-34.555-45.732-34.555-18.649 0-34.804 10.551-42.449 25.822-6.573-3.377-14.064-5.291-22.005-5.291-26.002 0-47.156 20.514-47.156 45.727v18.327c-5.305-2.018-11.075-3.126-17.106-3.126-25.95 0-47.062 20.514-47.062 45.758l.402 204.814c0 47.815 18.276 92.909 51.461 126.976 33.289 34.174 77.413 52.993 124.241 52.993 46.829 0 90.952-18.819 124.24-52.992 33.185-34.066 51.462-79.16 51.462-126.976 0 0 .017-83.809.036-152.852zm-30.036 152.852c0 39.949-15.254 77.609-42.95 106.042-27.594 28.326-64.085 43.926-102.752 43.926-38.666 0-75.157-15.6-102.751-43.926-27.696-28.433-42.95-66.093-42.95-106.071l-.402-204.814c0-8.673 7.654-15.729 17.062-15.729s17.061 7.056 17.061 15.729v100.56c0 8.284 6.76 15 15.044 15s15-6.716 15-15v-161.49c0-8.672 7.696-15.727 17.156-15.727s17.155 7.056 17.155 15.727v131.49c0 8.284 6.716 15 15 15h.143c8.284 0 15-6.716 15-15v-152.02c0-8.673 7.696-15.729 17.156-15.729s17.155 7.056 17.155 15.742l.143 152.02c.008 8.28 6.721 14.987 15 14.986h.007c8.281-.004 14.993-6.719 14.993-15v-121.224c0-8.672 7.696-15.727 17.156-15.727s17.155 7.056 17.155 15.727v191.225c0 8.284 6.716 15 15 15h.143c8.284 0 15-6.716 15-15v-88.569c0-8.673 7.696-15.729 17.156-15.729s17.155 7.056 17.155 15.725c-.007 28.48-.035 152.856-.035 152.856z" />
          <path d="m303.994 250.394c-5.857-5.857-15.355-5.857-21.213 0l-12.644 12.644-11.144-11.144c-5.857-5.857-15.355-5.857-21.213 0s-5.858 15.355 0 21.213l11.144 11.144-17.379 17.379-11.143-11.143c-5.857-5.857-15.355-5.857-21.213 0-5.858 5.858-5.858 15.355 0 21.213l11.143 11.143-17.379 17.379-11.143-11.143c-5.857-5.858-15.355-5.858-21.213 0-5.858 5.857-5.858 15.355 0 21.213l11.143 11.143-12.644 12.644c-5.858 5.857-5.858 15.355 0 21.213 2.929 2.929 6.768 4.394 10.606 4.394 3.839 0 7.678-1.465 10.607-4.394l12.644-12.643 11.143 11.143c2.929 2.93 6.768 4.394 10.606 4.394s7.678-1.465 10.607-4.394c5.858-5.857 5.858-15.355 0-21.213l-11.143-11.143 17.38-17.38 11.144 11.144c2.929 2.929 6.768 4.394 10.606 4.394s7.678-1.465 10.606-4.394c5.858-5.857 5.858-15.355 0-21.213l-11.144-11.144 17.379-17.379 11.144 11.144c2.929 2.929 6.767 4.394 10.606 4.394s7.678-1.465 10.606-4.394c5.858-5.857 5.858-15.355 0-21.213l-11.144-11.144 12.644-12.644c5.86-5.858 5.86-15.356.001-21.213z" />
        </g>
      </svg>
    ),
  },
  {
    title: "Recuperación más rápida",
    description:
      "Al ser la incisión menor, precisa de cuidados menos agresivos, lo que disminuye el tiempo de estancia hospitalaria del paciente.",
    icon: <img src="./images/png/thunder.png" alt="thunder icon"></img>,
  },
];

export const ocupationsData = [
  {
    icon: <img src="./images/png/doctor.png" alt="doctor icon" />,
    description:
      "Médico cirujano y especialista en cirugía laparoscópica. Egresado de la Universidad de Oriente, Venezuela.",
  },
  {
    icon: <img src="./images/png/pharmacy.png" alt="pharmacy icon" />,
    description: "Miembro titular de la Federación Médica Venezolana.",
  },
  {
    icon: <img src="./images/png/teacher.png" alt="teacher icon" />,
    description:
      "Profesor de Pregrado y Postgrado. Universidad de Oriente, núcleo Bolívar.",
  },
];

export const awardsData = [
  {
    icon: <img src="./images/png/medal.png" alt="medal icon" />,
    description: (
      <>
        <p>
          M.P.P.D y Centro Asistencial Militar TCNEL "Dr. Cesar Andrés Bello
          Descrivan" Agosto 2010.
        </p>
      </>
    ),
  },
  {
    icon: <img src="./images/png/graduation.png" alt="graduation icon" />,
    description: (
      <>
        <p>U.D.O 1991 - 1992.</p>
        <p>
          Especialización en cirugía laparoscópica básica y avanzada.
          <span>Valencia, Venezuela.</span>
        </p>
      </>
    ),
  },
  {
    icon: <img src="./images/png/trophy.png" alt="trophy icon" />,
    description: (
      <>
        <p>
          MP.P.S Centro Nefrológico "Dr Simplicio Hernández"{" "}
          <span>Tucupita, Edo. Delta Amacuro 2011.</span>
        </p>
      </>
    ),
  },
];

export const faqData = [
  {
    quest: "¿Cuando acudir al médico?",
    answer: (
      <p>
        {" "}
        Siempre que presentes síntomas o signos de cualquier patología y antes
        de cualquier pre medicación empírica a por sugerencia de cualquier otra
        persona ( amigos, familiares o conocidos) que podría eventualmente haber
        presentado los mismos síntomas, debes de acudir a tu medico de confianza
        para evaluación y conducta. Posterior a la evaluación el decidirá si los
        síntomas y signos (cuadro clínico) que presentas, puede ser tratada
        medicamente o debería solicitar la concurrencia de otras especialidades
        para el adecuado diagnóstico y tratamiento.
      </p>
    ),
  },
  {
    quest: "¿Que exámenes debo llevar a consulta?",
    answer: (
      <>
        {" "}
        <p>
          Si eres un paciente que presentas cualquier síntoma que te haga
          decidir consultar un especialista de cirugía general, se recomienda
          llevar estudios de laboratorio básicos.{" "}
        </p>
        <ul>
          <li>Hematología completa</li>
          <li>Formula y cuenta leucocitaria</li>
          <li>Glicemia en ayunas, creatinina y urea</li>
        </ul>
      </>
    ),
  },
  {
    quest: "¿Qué estudios de imágenes debería realizarme?",
    answer: (
      <p>
        Los estudio de imágenes aparte de los ya mencionados en la parte
        anterior, son estudios especializados que solo el médico tratante debe
        solicitar y va a depender de la patología o enfermedad que este
        sufriendo y van desde ultrasonido (ecosonograma) hasta estudios más
        complejos como tomografías simple o con doble contraste,
        colangioresonancia , o procedimiento diagnóstico y terapéuticos como la
        colangio-pancreatografia retrograda endoscópica (CEPRE), por ejemplo
        para litiasis vesicular y patología de la vías biliares el mejor estudio
        imageneologico es el ultrasonido (Ecosonograma), que tiene una
        probabilidad diagnostica del 90-98%, para caso donde hay dudas o
        queremos tener mayor información se solicitara el estudio más idóneo.
      </p>
    ),
  },
  {
    quest: "¿Qué debo tomar para calmar el dolor?",
    answer: (
      <p>
        Siempre sugerimos que un dolor abdominal (sin conocer las causa que lo
        origino) no debe ser tratado empíricamente con analgésicos ni con
        antibióticos , porque estos podrían enmascarar un cuadro clínico mucho
        peor y alterar la evolución y la aparición de síntomas y signos que
        serian de mucha ayuda a la hora de ser evaluada por el especialista.
      </p>
    ),
  },
  {
    quest:
      "En caso de requerir una operación, ¿cuál es la mejor opción, laparoscópica o abierta?",
    answer: (
      <>
        {" "}
        <h3>
          Recomendamos la cirugía laparoscópica siempre, por varios motivos:
        </h3>
        <ul>
          <li>
            Es una cirugía mínimamente invasiva, eso quiere decir que las manos
            del cirujano nunca tocan las estructuras internas del abdomen o
            cavidad pélvica.
          </li>
          <li>
            Mejor visualización de las estructuras y la posibilidad de evaluar
            toda la cavidad con una incisión de 0,5-1 cm.
          </li>
          <li>Menos dolor post-operatorio.</li>
          <li>
            Menos complicaciones a largo, mediano y corto plazo. La cirugía
            abierta puede presentar infección de la herida operatoria.
          </li>
          <li>Incorporación a sus actividades laborales de forma precoz.</li>
          <li>Y por supuesto las parte estética mucho más favorable.</li>
        </ul>
      </>
    ),
  },
];

export const methodsData = [
  {
    step: 1,
    description:
      "La mayoría de las laparoscopias se hacen bajo anestesia general. La anestesia general es un medicamento que deja al paciente profundamente dormido (inconsciente). Usted no sentirá ningún dolor durante la cirugía. El medicamento se administra por vía intravenosa (IV, por una vena) o inhalando gases a través de una máscara. Un médico llamado anestesiólogo administra este medicamento, si no le administran anestesia general, le inyectarán un medicamento en el abdomen para adormecer el área para que no sienta ningún dolor.",
  },
  {
    step: 2,
    description:
      "Una vez que esté inconsciente o su abdomen esté completamente adormecido, el cirujano le hará una incisión pequeña justo debajo del ombligo o cerca de esa región, el laparoscopio, un tubo delgado con una cámara en un extremo, se inserta a través de la incisión. Se pueden hacer más incisiones pequeñas si se necesita una sonda u otras herramientas quirúrgicas.",
  },
  {
    step: 3,
    description:
      "Durante el procedimiento, se coloca un tipo de gas en el abdomen. Esto expande la región y permite que el cirujano vea el interior del cuerpo, el cirujano moverá el laparoscopio alrededor de esa región. Mirará las imágenes del abdomen y de los órganos pélvicos en una pantalla de computadora.",
  },
  {
    step: 4,
    description:
      "Cuando el procedimiento haya terminado, se quitarán las herramientas quirúrgicas y la mayor parte del gas. Las incisiones pequeñas se cerrarán, usted será trasladado a una sala de recuperación.",
  },
  {
    step: 5,
    description:
      "Después de la laparoscopia, tal vez sienta sueño o náuseas por unas horas.",
  },
];

export const establishmentImages = [
  {
    name: "consulta médica",
    url: "./images/jpg/consultorio-1.jpg",
  },

  {
    name: "sala de operación",
    url: "./images/jpg/sala-operacion-1.jpg",
  },
  {
    name: "camilla de operacion",
    url: "./images/jpg/camilla-operacion.jpg",
  },
  {
    name: "consultorio vacío",
    url: "./images/jpg/consultorio-2.jpg",
  },
  {
    name: "equipos",
    url: "./images/jpg/equipos.jpg",
  },
  {
    name: "silla quirúrgica",
    url: "./images/jpg/sala-operacion-2.jpg",
  },
];
export const establishmentImages2 = [
  {
    name: "doctor giuseppe operando",
    url: "./images/jpg/operacion-2.jpg",
  },
  {
    name: "sala de operación",
    url: "./images/jpg/operacion-1.jpg",
  },
  {
    name: "camillas",
    url: "./images/jpg/sala-operacion-3.jpg",
  },
  {
    name: "sala de operaciones",
    url: "./images/jpg/sala-operacion-4.jpg",
  },
  {
    name: "consultorio frente",
    url: "./images/jpg/consultorio-frente.jpg",
  },
];
export const socialNetworks = [
  {
    network: "Facebook",
    url: "https://www.facebook.com/pepecoiro",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
        <path
          fill="#837D88"
          d="M22.675 0H1.325C.593 0 0 .593 0 1.325v21.351C0 23.407.593 24 1.325 24H12.82v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116c.73 0 1.323-.593 1.323-1.325V1.325C24 .593 23.407 0 22.675 0z"
        />
      </svg>
    ),
  },
  {
    network: "Instagram",
    url: "https://www.instagram.com/gcoiro15",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24">
        <path
          fill="#837D88"
          d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"
        />
      </svg>
    ),
  },
  {
    network: "Twitter",
    url: "https://www.twitter.com/gcoiro15",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" width="24" height="20">
        <path
          fill="#837D88"
          d="M24 2.557a9.83 9.83 0 01-2.828.775A4.932 4.932 0 0023.337.608a9.864 9.864 0 01-3.127 1.195A4.916 4.916 0 0016.616.248c-3.179 0-5.515 2.966-4.797 6.045A13.978 13.978 0 011.671 1.149a4.93 4.93 0 001.523 6.574 4.903 4.903 0 01-2.229-.616c-.054 2.281 1.581 4.415 3.949 4.89a4.935 4.935 0 01-2.224.084 4.928 4.928 0 004.6 3.419A9.9 9.9 0 010 17.54a13.94 13.94 0 007.548 2.212c9.142 0 14.307-7.721 13.995-14.646A10.025 10.025 0 0024 2.557z"
        />
      </svg>
    ),
  },
];
